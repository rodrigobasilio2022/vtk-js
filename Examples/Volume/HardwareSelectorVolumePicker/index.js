...
// ----------------------------------------------------------------------------
// Add a surface mesh (cube) so we can test picking
// against both volume and polydata.
// ----------------------------------------------------------------------------
...
function pickWorldPosition(x, y, tolerance = 5) {
  ...
  if (!info || info.prop == null || info.zValue === undefined) {
    return null;
  }

  const worldPosition = openGLRenderWindow.displayToWorld(
...
renderWindow.getInteractor().onRightButtonPress((callData) => {
  ...
  const world = pickWorldPosition(pos.x, pos.y, 5);
  if (!world) {
    return;
  }
